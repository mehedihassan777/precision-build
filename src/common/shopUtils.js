...
export const checkProductInCompare = (compareArr, productId) => {
  return compareArr.find((item) => item.id === productId);
};
...
export const calculateTotalOrder = (cartArr, discount, ship, isformatCurrency) => {
  let total = 0;
  let totalOrder = 0;
  cartArr.forEach((item) => (total += item.price * item.cartQuantity));
  totalOrder = total - discount + ship;
  return isformatCurrency ? formatCurrency(totalOrder) : totalOrder;
};
