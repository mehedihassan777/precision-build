...
const ServiceFour = ({ data, start, limit }) => {
    return (
        <section className="section services style-four sm:py-25 py-15 bg-dark">
            <div className="container flex flex-col items-center">
                <div className="heading flex max-lg:flex-col justify-center gap-7 gap-y-5 text-white">
                    <div className="lg:w-1/2 animate animate_right">
                        <span className="section_tag tag bg-white">What We Do?</span>
            ...
            <div className="services_list w-full lg:mt-12 mt-9 lg:px-10 px-4">
                <Swiper
                    ...
                    modules={[Pagination]}
                    className='swiper-pagination-relative swiper-pagination-white text-center sm:-mx-10'
                    breakpoints={{
                        576: {
                            slidesPerView: 2,
                            spaceBetween: -20,
                ...
                >
                    {data.slice(start, limit).map((item, index) => (
                        <SwiperSlide key={index} className="services_item">
                            <ServiceItem data={item} type={'style-four'} />
                        </SwiperSlide>
                    ))}
                </Swiper>
            </div>
